...
def _build_idf_table(counts):
    num_documents = counts["num_functions"]
    return MappingProxyType(
        {
            term: math.log((num_documents - max(term_count, 1)) / max(term_count, 1))
            for term, term_count in counts.items()
            if term != "num_functions"
        }
    )
...
class MnemonicTfIdf:
    ...
    _idf_by_bitness = MappingProxyType(
        {
            32: _build_idf_table(all_histograms_x86),
            64: _build_idf_table(all_histograms_x64),
        }
    )
    _max_idf_by_bitness = MappingProxyType({bitness: max(idf.values()) for bitness, idf in _idf_by_bitness.items()})

    def __init__(self, bitness=32):
        bitness = 32 if bitness == 32 else 64
        self.idf = self._idf_by_bitness[bitness]
        self._max_idf = self._max_idf_by_bitness[bitness]
