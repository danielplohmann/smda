"""Architecture-neutral function-start candidate.

Carries the candidate bookkeeping shared by every native backend: inbound call
references, evidence flags (symbol / exception record / stub / language spec /
gap / tailcall), priority-queue scoring and confidence estimation. The only
architecture-specific decision — whether the candidate's first bytes look like
a function entry — is delegated to :meth:`getFunctionStartScore`, which each
backend's subclass implements against its own prologue definitions.
"""
...
class FunctionCandidate:
    #: number of bytes captured at the candidate address for entry-shape checks;
    #: subclasses size this to their architecture's widest prologue window
    #: (intel: widest COMMON_PROLOGUES entry + endbr64; aarch64: one instruction
    #: word). The base value is only a fallback for the abstract class, which is
