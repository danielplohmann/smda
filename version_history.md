 ...
 * 2026-06-24: v4.2.0 - AArch64 `escapeBinary` is now per-mnemonic immediate-aware (nibble-keep-mask), giving a position-invariant `pic_hash` under relocation, mirroring the intel escaper. New `tests/testAArch64EscaperImproved.py` covers handcrafted instructions for the affected mnemonics (`adrp`, `adr`, `b`, `bl`, `b.cond`, `cbz`, `cbnz`, `tbz`, `tbnz`, `ldr`, `str`, `ldp`, `stp`, `ldur`, `stur`, `ldrb`, `ldrh`, `ldrsb`, `ldrsh`, `strb`, `strh`, `ldnp`, `stnp`, `ldpsw`, `add`, `sub`, `adds`, `subs`, `movz`, `movk`, `movn`, `mov`). Bumped `AARCH64_PIC_HASH_ESCAPE_VERSION` from [4,1,0] to [4,2,0] so existing reports are recomputed on load. `validation/inspect_escaping.py` now reports per-arch column widths and an ESCBIN/ESCOPC mismatch count.
