#!/usr/bin/python
"""AArch64 Mach-O corpus fixtures derived from Malpedia.
...
CORPUS_DIR = Path(__file__).resolve().parent / "aarch64_malpedia_macho_corpus"
...
class TestAArch64MalpediaMachoCorpus(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_manifest_records_curated_malpedia_subset(self):
        self.assertEqual(self.manifest["source"], "Malpedia")
        ...
        self.assertEqual(len({fixture["family"] for fixture in self.fixtures}), len(self.fixtures))
        for fixture in self.fixtures:
            with self.subTest(fixture=fixture["fixture"]):
                self.assertEqual(fixture["malpedia_commit"], self.manifest["malpedia_commit"])
                self.assertTrue(fixture["inner_path"].endswith("_unpacked"))
    ...
    def test_selected_malpedia_macho_fixtures_disassemble(self):
        for fixture in self.fixtures:
            with self.subTest(fixture=fixture["fixture"]):
